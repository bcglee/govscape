...
class PDFsToEmbeddings:
    ...
    def __init__(self, pdf_directory, txt_directory, jpgs_dir, e_jpgs_dir, embeddings_dir, embeddings_img_dir, embeddings_extract_dir):
        ...
        print(self.jpgs_path)
        print(self.extracted_jpgs_path)
        print(self.embeddings_img_path)
        print(self.embeddings_img_e_path)
    ...
    # single txt -> embed
    def convert_txt_to_embedding(self, txt_path, text_model):
        # need to convert .txt to text
        text = self.txt_to_text(txt_path)
        return text_model.encode_text(text)

    
    # version 1 = single subdir: txt subdir -> embed subdir.
    def convert_subdir_to_embeddings(self, txt_subdir_path, text_model):
        ...
        for txt_file in txt_files:
            ...
            embedding = self.convert_txt_to_embedding(txt_path, text_model)
    ...
    # 1. OG VERSION 
    def convert_txts_to_embeddings(self, pdf_files, text_model):
        self.ensure_dir(self.embeddings_path)
        for pdf_file in pdf_files:
            subdir = os.path.join(self.txts_path, os.path.splitext(pdf_file)[0])
            if os.path.isdir(subdir):
                self.convert_subdir_to_embeddings(subdir, text_model)
